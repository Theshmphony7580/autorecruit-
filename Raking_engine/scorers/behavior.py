...
from utils.safe_extract import safe_float, safe_int, safe_bool
...
def get_behavior_score(candidate: dict) -> float:
    ...
    open_to_work = 1.0 if safe_bool(signals.get('open_to_work_flag', False)) else 0.0
    ...
    raw_github = safe_int(signals.get('github_activity_score', -1), -1)
    ...
    raw_response_hours = safe_float(signals.get('avg_response_time_hours', BEHAVIOR_RESPONSE_TIME_CEILING_HOURS), BEHAVIOR_RESPONSE_TIME_CEILING_HOURS)
    ...
    if assessment_dict and isinstance(assessment_dict, dict):
        scores = [safe_float(v) for v in assessment_dict.values()]
    ...
    raw_notice = safe_float(signals.get('notice_period_days', BEHAVIOR_NOTICE_PERIOD_CEILING_DAYS), BEHAVIOR_NOTICE_PERIOD_CEILING_DAYS)
